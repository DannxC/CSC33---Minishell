...
void parse_antigo(char *input, char **command, char ***(args), char **inputFile, char **outputFile)
{
...
void parse_input(char *input, Command *commands, int *numCommands)
{
    ...
    *numCommands = 0;
    ...
    // Parse each segment of the input separated by '|'
    while (segment != NULL && *numCommands < MAX_COMMANDS)
    {
        parse_single_command(segment, &commands[*numCommands]);
        segment = strtok(NULL, "|");
        (*numCommands)++;
    }
}
